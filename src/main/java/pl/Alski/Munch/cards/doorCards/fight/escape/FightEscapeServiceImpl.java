...
public class FightEscapeServiceImpl implements FightEscapeService {
    ...
    private final DiceModifierService diceModifierService;
    ...
    public boolean tryToEscapeFromAllMonsters(Player player, Fight fight) {
        ...
        for (Monster monster: fight.getMonsters()){
            boolean result = tryToEscapeFromMonster(player, monster);
            if (result) {
                ...
                eventService.makeEventHappenToPlayer(player, monster.getMiserableEnd());
            }
        }
    }

    public boolean tryToEscapeFromMonster(Player player, Monster monster) {
        if (conditionService.checkCondition(player, monster.getCanRunFromFightWithoutDiceThrow())) return true;
